...
- 逆に「ユーザーからの追加指示」で日付・候補日を含めない、消す、書かないことが求められた場合は、受信メールが日程調整を求めているかどうかに関わらずそれを最優先し、${CANDIDATES_MARKER}セクション自体を省略し、${BODY_MARKER}中にも具体的な日付・時間帯を一切書かないこと(${EARLIEST_DATE_MARKER}も同様に省略する)
...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (!apiKey) {
    return jsonResponse(res, 500, { error: "Anthropic API is not configured on the server" });
  }
