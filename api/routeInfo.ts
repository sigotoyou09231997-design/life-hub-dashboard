...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (!apiKey) {
    ...
    return jsonResponse(res, 200, { configured: false } satisfies RouteInfoResponse);
  }

  const payload = (typeof req.body === "string" ? JSON.parse(req.body || "{}") : (req.body ?? {})) as RouteInfoBody;
  const origin = payload.origin?.trim();
  const destination = payload.destination?.trim();
  if (!origin || !destination) return jsonResponse(res, 400, { error: "origin and destination are required" });
  ...
  return jsonResponse(res, 200, { configured: true, modes } satisfies RouteInfoResponse);
};
