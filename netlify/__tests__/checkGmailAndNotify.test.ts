...
describe("buildNotificationPayload", () => {
  it("uses the sender's display name as the title, and subject+snippet as the body", () => {
    ...
    expect(payload.body).toBe("会議の件\n来週の会議についてご相談が...");
    expect(payload.url).toBe("/gmail");
  });

  it("falls back to the raw address when the From header has no display name", () => {
    const payload = JSON.parse(buildNotificationPayload(1, { from: "a@example.com", subject: "件名", snippet: "" }));
    expect(payload.title).toBe("a@example.com");
    expect(payload.body).toBe("件名");
  });
