 ...
 * ブラウザに出さないため(netlify/functions/routeInfo.ts と同じ理由)。
 *
...
const handler: Handler = async (event) => {
  ...
  if (!apiKey) return jsonResponse(200, { configured: false } satisfies NearestStationResponse);

  let address = "";
  try {
    address = String((JSON.parse(event.body ?? "{}") as { address?: unknown }).address ?? "").trim();
  } catch {
    return jsonResponse(400, { error: "Invalid JSON" });
  }
  if (!address) return jsonResponse(400, { error: "address is required" });
  ...
  try {
    ...
    if (!location) return jsonResponse(200, { configured: true } satisfies NearestStationResponse);
    ...
    if (!station) return jsonResponse(200, { configured: true } satisfies NearestStationResponse);
