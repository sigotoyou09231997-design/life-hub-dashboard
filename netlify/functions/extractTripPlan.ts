 ...
 * 別々に置く決まり)。片方だけ直して食い違わないよう、
 * netlify/__tests__/extractTripPlan.test.ts で両者が同じ結果を返すことを突き合わせている。 */
...
/** 資料をAnthropicへ渡すのは下書き生成(generateDraft.ts)と同じで、鍵をブラウザに
 * 出さないためのサーバー経由。 */
export const handler: Handler = async (event) => {
  ...
  if (imageError) {
    return jsonResponse(400, { error: imageError });
  }
  if (hasNoSource(payload, images.length)) {
    return jsonResponse(400, { error: "読み取るもとになる文章か写真が必要です" });
  }
  ...
  if (data.stop_reason === "max_tokens") {
    ...
    // 見えてしまう。理由が分かる形で伝える。
    return jsonResponse(502, { error: "内容が多すぎて読み取りきれませんでした。分けてもう一度お試しください" });
  }
